...
// --- END NEW UPDATES (नई अपडेट्स समाप्त) ---
        ...
        // --- (मौजूदा कोड को बदले बिना जोड़ा गया) ---
        // ------------------------------------------------------------------
        ...
        // (नोट: सटीक रिपोर्टिंग के लिए POST /api/invoices को इन्हें सेव करने के लिए बदलना होगा, 
        // लेकिन "नो चेंज" नियम के कारण, हम मौजूदा कोड को नहीं बदल रहे हैं।)
        await client.query(`
            DO $$ BEGIN
...
app.post('/api/admin/generate-key', async (req, res) => {
    ...
    if 
(adminPassword !== process.env.GLOBAL_ADMIN_PASSWORD) {
...
const token = jwt.sign(tokenUser, JWT_SECRET, { expiresIn: '30d' });
        ...
        const expiryDate = user.license_expiry_date ?
new Date(user.license_expiry_date) : null;
...
app.post('/api/activate-license', authenticateJWT, async (req, res) => {
    ...
    const keyHash = hashKey(licenseKey); // कुंजी को हैश 
करें
    const client = await pool.connect();
...
app.post('/api/users', authenticateJWT, checkRole('ADMIN'), async (req, res) => { 
   ...
   try {
        ...
        res.json({ success: 
true, user: result.rows[0], message: 'यूजर सफलतापूर्वक जोड़ा गया।' });
    } catch (err) {
...
message: 'स्टॉक आइटम प्राप्त करने में विफल।' });
    }
});
...
app.post('/api/expenses', authenticateJWT, checkRole('MANAGER'), async (req, res) => { // Manager and above
    ...
    if (isNaN(safeAmount) || safeAmount <= 0) {
        return 
res.status(400).json({ success: false, message: 'राशि एक मान्य धनात्मक संख्या होनी चाहिए।' });
    }
...
app.get('/api/backup', authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        for (const table of tables) {
            const result = await client.query(`SELECT * FROM ${table} WHERE shop_id = 
$1`, [shopId]);
...
await client.query(
        ...
        if (checkResult.rows.length > 0) {
            await client.query('ROLLBACK');
            return res.status(400).json({ success: false, message: 'आज की क्लोजिंग पहले ही 
रन हो चुकी है।' });
        }
